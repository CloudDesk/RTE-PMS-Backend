# Overall Reality - Static Roles Phase

This review is based on the current development instruction:

> Current implementation can proceed with static PMS roles, but dynamic role configuration still needs to be implemented later to fully satisfy the FSD.

Because of this, the completion percentage should be read in two ways:

- **Static-role MVP completion:** how complete the current agreed implementation phase is.
- **Full FSD completion:** how complete the system is against `PMS_FSD_v2`, including dynamic roles.

## Overall Completion

| View | Completion | Meaning |
|---|---:|---|
| Static-role MVP / Director demo readiness | 70-75% | Core PMS lifecycle is mostly demo-ready with controlled data and static roles |
| Full FSD completion | 60-65% | Dynamic roles, full access governance, SLA, reporting, QA, and communication mapping still need work |
...
| Static Access Foundation | 65-70% | 55-60% | Static role flow exists for MVP |
| Dynamic Access Engine | 25-35% | 20-30% | Dynamic role configuration not implemented now |
...
- Dynamic role configuration is available, not only static role checks

That is not 100% yet.

For the current static-role phase, the system can be considered **70-75% demo/MVP ready**, but not full FSD complete.

## Static Role Clarification

Current accepted static PMS role mapping:

| HRMS Role | PMS Meaning |
|---|---|
| staff / employee | Employee |
| manager | Manager |
| admin | HR/Admin |
| management / director | Senior decision role, where available |

This is acceptable for the current MVP phase if the team documents it clearly.

However, `PMS_FSD_v2` expects a dynamic access model:

```text
Role + Assignment + Hierarchy Scope + Workflow State + Section/Field Visibility
```

So dynamic roles are **not completed**. They should be tracked as pending FSD work.
...
### Static Role Phase Note

For the current phase, role behavior can be demonstrated using static roles like Employee, Manager, Admin, Management, and Director.

For full FSD, dynamic role configuration still needs to be implemented.
...
- Is dynamic role configuration tracked as pending work?
...
- Where will dynamic role configuration UI be added later?
...
Static access foundation should support:

- Admin access
- Employee self-access
- Manager assigned-employee access
- Basic senior role access where configured
- Basic role-aware navigation
- Server-side access checks for major PMS APIs
...
- Static role checks are not a replacement for full dynamic access
- Field-level permission enforcement across all APIs needs proof
- Hierarchy scopes beyond assigned manager need proof
- Delegation needs to be consistently included in access checks

FE:

- UI actions must be driven by actual backend access result
- Access simulator must match real API behavior
- Need clearer access-denied messaging

### Questions To Ask Team

BE questions:

- Are all major PMS APIs protected by server-side static role checks?
- Does manager access depend on assignment ownership?
- Can employee access only own PMS data?
- Are director/management access rules clearly defined?
- Is delegation included in access calculation everywhere?

FE questions:

- Does UI hide actions based on actual backend permission?
- Can employee see only own assignments?
- Can manager see only assigned employees?
- Can HR/Admin see all governed records?
- Does UI clearly explain access-denied cases?

## 11. Dynamic Access Engine
...
Dynamic role configuration is **not implemented now**.

The current implementation uses static roles for the MVP phase. This is acceptable for the current development instruction, but it is not full FSD completion.

### What Is Done

BE:

- Basic static access foundation exists
- Some template visibility metadata exists
- Some delegation checks exist

FE:

- Access simulator exists
- Template visibility/editability controls exist
...
- No HR-configurable PMS role setup
...
- Where will PMS roles be configured?
- How will HR map HRMS roles to PMS roles?
...
- Where can HR configure section/field permission matrix?
...
## 16. Build / QA / Production Readiness
...
> The implementation is not 100% complete against PMS_FSD_v2. For the current static-role MVP phase, the system is around 70-75% demo-ready. A strong foundation exists for template, cycle, assignment, objective, review, and annual decision flows. However, full FSD completion is still around 60-65% because dynamic role configuration is not implemented, communication outcome mapping is not fully aligned with dispatch, visibility governance needs full API-level proof, SLA/delegation/reporting are partial, FE check/build is not clean, and full end-to-end QA coverage is missing.
